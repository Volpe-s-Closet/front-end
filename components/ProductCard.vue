<template>
  ...
  <div v-if="viewMode === 'grid'"
    ...
    <div class="relative">
      <img :src="getProductImage(product)" :alt="product.name"
        class="w-full h-64 object-cover cursor-pointer hover:opacity-90 transition-opacity" @error="handleImageError"
      ...
      <!-- Zoom icon overlay -->
      <div
        class="absolute inset-0 flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity bg-black bg-opacity-20 pointer-events-none">
    ...
    <button v-if="product.purchasable && product.stock_status === 'instock'"
      @click="hasVariations ? openVariationModal() : handleAddToCart()" :disabled="isAdding"
    ...
    <NuxtLink :to="`/product/${product.slug}`" class="block p-4 hover:bg-gray-50 transition-colors cursor-pointer">
      <h3
        class="text-lg font-semibold text-gray-900 mb-2 overflow-hidden text-ellipsis line-clamp-2 hover:text-blue-600 transition-colors">
        {{ product.name }}
      </h3>
      ...
      <div class="flex items-center mb-3">
        <div class="flex text-yellow-400">
          <Icon v-for="i in 5" :key="i" name="heroicons:star-solid" :class="[
            'h-4 w-4',
      ...
      <p v-if="product.short_description" class="text-gray-600 text-sm mb-4 overflow-hidden text-ellipsis line-clamp-2"
    ...
    <!-- Variation Modal -->
    <ProductVariationModal :product="product" :is-open="isVariationModalOpen" @close="closeVariationModal"
  ...
  <div v-else
    ...
    <div class="flex flex-col sm:flex-row">
      ...
      <NuxtLink :to="`/product/${product.slug}`"
        class="flex-1 p-6 block hover:bg-gray-50 transition-colors cursor-pointer">
        <div class="flex flex-col sm:flex-row sm:justify-between h-full">
          <div class="flex-1">
            ...
            <p v-if="product.short_description" class="text-gray-600 mb-4 overflow-hidden text-ellipsis line-clamp-3"
              v-html="product.short_description"></p>
  ...
  <!-- Image Gallery Modal -->
  <div v-if="showImageGallery" class="fixed inset-0 z-50 bg-black bg-opacity-90 flex items-center justify-center p-4"
    @click="closeImageGallery">
    <div class="relative max-w-4xl max-h-full w-full">
      <!-- Close Button -->
      <button @click="closeImageGallery"
        class="absolute -top-2 -right-4 z-10 bg-black bg-opacity-50 text-white p-2 rounded-full hover:bg-opacity-75 transition-colors"
      ...
      <!-- Gallery Component -->
      <div @click.stop>
        <ProductImageGallery :product="product" :selected-variation="null" />
      </div>
    </div>
  </div>
</template>
