<template>
  <div class="bg-white rounded-lg shadow-sm p-6">
    <h2 class="text-xl font-semibold mb-6">{{ $t('checkout.contact.title') }}</h2>
    ...
    <template v-if="isMounted">
      ...
      <div v-if="isLoadingUserData" class="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-md">
        <div class="flex items-center">
          ...
          <span class="text-sm text-blue-700">{{ $t('checkout.contact.loadingAddresses') }}</span>
        </div>
      </div>
      ...
      <div v-else-if="isAuthenticated && checkoutData.billing.address_1"
        ...
        <div class="flex items-center">
          ...
          <span class="text-sm text-green-700">{{ $t('checkout.contact.savedLoaded') }}</span>
        </div>
      </div>
      ...
      <div v-else-if="!isAuthenticated" class="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-md">
        <div class="flex items-center justify-between">
          <div class="flex items-center">
            ...
            <span class="text-sm text-blue-700">{{ $t('checkout.contact.loginPrompt') }}</span>
          </div>
          <NuxtLink to="/login?redirect=/checkout"
            class="text-sm font-medium text-blue-600 hover:text-blue-500">
            {{ $t('checkout.contact.logIn') }}
          </NuxtLink>
        </div>
      </div>
    </template>
    ...
    <form @submit.prevent="$emit('proceed')" class="space-y-4">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-2">{{ $t('checkout.contact.firstName') }} *</label>
          <input v-model="checkoutData.billing.first_name" type="text" required
            class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent">
        </div>
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-2">{{ $t('checkout.contact.lastName') }} *</label>
      ...
      <div>
        <label class="block text-sm font-medium text-gray-700 mb-2">{{ $t('checkout.contact.email') }} *</label>
        <input v-model="checkoutData.billing.email" type="email" required
          class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent">
      </div>

      <div>
        <label class="block text-sm font-medium text-gray-700 mb-2">{{ $t('checkout.contact.phone') }}</label>
        <input v-model="checkoutData.billing.phone" type="tel"
          class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent">
      </div>

      <div>
        <label class="block text-sm font-medium text-gray-700 mb-2">{{ $t('checkout.contact.company') }}</label>
        <input v-model="checkoutData.billing.company" type="text"
          class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent">
      </div>

      <h3 class="text-lg font-semibold mt-8 mb-4">{{ $t('checkout.contact.billingAddress') }}</h3>

      <div>
        <label class="block text-sm font-medium text-gray-700 mb-2">{{ $t('checkout.contact.addressLine1') }} *</label>
        <input v-model="checkoutData.billing.address_1" type="text" required
          class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent">
      </div>

      <div>
        <label class="block text-sm font-medium text-gray-700 mb-2">{{ $t('checkout.contact.addressLine2') }}</label>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-2">{{ $t('checkout.contact.city') }} *</label>
          <input v-model="checkoutData.billing.city" type="text" required
            class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent">
        </div>
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-2">{{ $t('checkout.contact.state') }} *</label>
          <input v-model="checkoutData.billing.state" type="text" required
            class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent">
        </div>
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-2">{{ $t('checkout.contact.zip') }} *</label>
      ...
      <div>
        <label class="block text-sm font-medium text-gray-700 mb-2">{{ $t('checkout.contact.country') }} *</label>
        <SelectBox
          v-model="checkoutData.billing.country"
          :options="countryOptions"
          :placeholder="$t('checkout.contact.selectCountry')"
          button-class="w-full"
        />
      </div>
      ...
      <div class="mt-6">
        <label class="flex items-center">
          ...
          <span class="ml-2 text-sm text-gray-700">{{ $t('checkout.contact.shipSameAddress') }}</span>
        </label>
      </div>

      <div v-if="!sameAsShipping" class="space-y-4">
        <h3 class="text-lg font-semibold mt-6 mb-4">{{ $t('checkout.contact.shippingAddress') }}</h3>
      ...
      <BaseButton action="submit" :text="$t('checkout.contact.continueToPayment')" full-width size="lg" />
    </form>
  </div>
</template>
...
// Country options aren't translated since the country names belong to the
// destination locale (e.g. you shouldn't pick "United States" in Italian).
// Keep them in their natural form.
